...
def test_normalize_header_collapses_spaces() -> None:
    assert normalize_header("Dosage Forms/ Regulatory Status") == "Dosage Forms/Regulatory Status"


def test_split_sections_maps_known_headers() -> None:
    body = "\n".join(
        [
            "Uses/Indications",
...
def test_overdosage_variant_maps_to_overdose_toxicity() -> None:
    body = "Overdosage/Acute Toxicity\nSupportive care."
    sections = split_sections(body)
    assert sections[0].section_type == SectionType.OVERDOSE_TOXICITY


def test_unknown_header_is_not_treated_as_section() -> None:
